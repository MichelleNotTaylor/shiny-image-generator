...
def main(size, invaders, imgSize): 
    ...
    for x in range(0, invaders):
        for y in range(0, invaders):
            ...
            create_alien((topLeftX, topLeftY, botRightX, botRightY), draw, size)

            origImage.save("Aliens/alien-" + str(size) + "x" + str(size) + "-" + str(invaders) + "-" + str(imgSize) + ".jpg")
...
def create_alien(border, draw, size): 
    x0, y0, x1, y1 = border
    squareSize = (x1-x0)/size
    randColors = [rc(), rc(), rc(), (0,0,0), (0,0,0)] 
    i = 1
